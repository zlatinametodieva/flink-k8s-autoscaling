...
public final class OrderedProperties implements Serializable {
    ...
    public PropertyTree getPropertyList(String key) {

        PropertyTree base = PropertyTree.of(key);
        String[] target = key.split("\\.");
        for (Map.Entry<String, String> entry : properties.entrySet()) {

            String[] current = entry.getKey().split("\\.");
            ...
            if (matches) {
                ...
                PropertyTree property = PropertyTree.of(entry.getKey(), entry.getValue());
                PropertyTree.merge(base.getRoot(), property.getRoot());
