package de.tu_berlin.dos.arm.archon.common.data;
...
public class TimeSeries implements Serializable {
    ...
    public TimeSeries alignedMerge(TimeSeries that) {

        if (!this.getFirst().equals(that.getFirst()) ||
            !this.getLast().equals(that.getLast()) ||
            this.size() != that.size()) {

            throw new IllegalStateException("Unable to merge time series");
        }
        for (Observation thisObs : this.observations) {

            Observation thatObs = that.getObservation(thisObs.timestamp);
            if ((thisObs.value != null) && (thatObs.value != null)) {

                this.setObservation(new Observation(thisObs.timestamp, thisObs.value + thatObs.value));
            }
            else if (thatObs.value != null) {

                this.setObservation(new Observation(thatObs.timestamp, thatObs.value));
            }
            else if (thisObs.value != null) {

                this.setObservation(new Observation(thisObs.timestamp, thisObs.value));
            }
            else {

                this.setObservation(new Observation(thisObs.timestamp, null));
            }
        }
        return this;
    }

    public CompletableFuture<TimeSeries> asyncUnalignedMerge(Executor executor, TimeSeries that) {

        return CompletableFuture.supplyAsync(() -> this.unalignedMerge(that), executor);
    }

    public TimeSeries unalignedMerge(TimeSeries that) {

        long first, last;
        if (this.getFirst().timestamp < that.getFirst().timestamp) first = this.getFirst().timestamp;
        else first = that.getFirst().timestamp;
        if (this.getLast().timestamp > that.getLast().timestamp) last = this.getLast().timestamp;
        else last = that.getLast().timestamp;

        TimeSeries newTs = TimeSeries.create(first, last);
        @SuppressWarnings("unchecked")
        LinkedList<Observation> newThis = (LinkedList<Observation>) SerializationUtils.clone(this.observations);
        @SuppressWarnings("unchecked")
        LinkedList<Observation> newThat = (LinkedList<Observation>) SerializationUtils.clone(that.observations);
        for (Observation curr : newTs.observations) {

            int thisIdx = newThis.indexOf(new Observation(curr.timestamp, null));
            int thatIdx = newThat.indexOf(new Observation(curr.timestamp, null));
            if (thisIdx != -1 && thatIdx == -1) newTs.setObservation(newThis.get(thisIdx));
            else if (thisIdx == -1 && thatIdx != -1) newTs.setObservation(newThat.get(thatIdx));
            else if (thisIdx != -1 && thatIdx != -1) {

                Double sum = null;
                if (newThis.get(thisIdx).value != null && newThat.get(thatIdx).value == null) {

                    sum = newThis.get(thisIdx).value;
                }
                else if (newThis.get(thisIdx).value == null && newThat.get(thatIdx).value != null) {

                    sum = newThat.get(thatIdx).value;
                }
                else if (newThis.get(thisIdx).value != null && newThat.get(thatIdx).value != null) {

                    sum = newThis.get(thisIdx).value + newThat.get(thatIdx).value;
                }
                newTs.setObservation(new Observation(curr.timestamp, sum));
            }
        }
        this.observations = newTs.observations;
        return this;
    }
