...
import de.tu_berlin.dos.arm.archon.common.clients.prometheus.PrometheusClient.Matrix.MatrixData.MatrixResult;
import de.tu_berlin.dos.arm.archon.common.data.Observation;
...
public class PrometheusClient {
    ...
    public String takeSnapshot() throws IOException {
        ...
        JsonObject response = this.service.takeSnapshot().execute().body();
        if (response != null && response.get("status").getAsString().equals("success")) {
            snapshot = response.getAsJsonObject("data").get("name").getAsString();
        }
        return snapshot;
    }
}
